...
// Or vice versa
int AINGM::presetSearchBreadth[5] = {17, 7, 5, 3, 3};
...
void AINGM::heuristicNegamax(const char *gs, int player, int depth, int time_limit, bool enable_ab_pruning,
...
int AINGM::heuristicNegamax(char *gs, int player, int initial_depth, int depth,
    ...
    if (moves_player.size() == 1 || moves_player[0].heuristic_val >= AiEvalWinningScore) {
    ...
    if (moves_opponent[0].heuristic_val >= AiEvalThreateningScore) {
        ...
        for (int i = 0; i < tmp_size; ++i) {
            ...
            move.heuristic_val = AIEval::evalMove(gs, move.r, move.c, player);
...
//    }
    ...
    for (int i = 0; i < size; ++i) {
        ...
        // Execute move
        AIUtils::setCell(gs, move.r, move.c, static_cast<char>(player));
...
//            std::cout << depth << " | " << move.r << ", " << move.c << ": " << move.actual_score << std::endl;
        ...
        // Restore
        AIUtils::setCell(gs, move.r, move.c, 0);
...
void AINGM::searchMovesOrdered(const char *gs, int player, std::vector<Move> *result) {
    // Clear and previous result
    result->clear();

    // Find an extent to reduce unnecessary calls to AIUtils::remoteCell
    ...
    for (int r = min_r - 2; r <= max_r + 2; ++r) {
        for (int c = min_c - 2; c <= max_c + 2; ++c) {
            ...
            if (AIUtils::remoteCell(gs, r, c)) continue;
            ...
            // Evaluate move
            m.heuristic_val = AIEval::evalMove(gs, r, c, player);
...
int AINGM::negamax(char *gs, int player, int depth, int *move_r, int *move_c) {
    // Initialize with a minimum score
    int max_score = INT_MIN;

    // Eval game state
    if (depth == 0) return AIEval::evalState(gs, player);

    // Loop through all cells
    for (int r = 0; r < g_board_size; ++r) {
        for (int c = 0; c < g_board_size; ++c) {
            // Consider only empty cells
            if (AIUtils::getCell(gs, r, c) != 0) continue;

            // Skip remote cells (no pieces within 2 cells)
            if (AIUtils::remoteCell(gs, r, c)) continue;

            // Execute move
            AIUtils::setCell(gs, r, c, static_cast<char>(player));
